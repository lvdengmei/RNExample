...
let left_width = SCREEN_WIDTH;//在一次完整事件中左组件的宽度
let right_width = 0;//在一次完整事件中右组件的宽度
let x = 0;//移动时的位移 大于dx x = delete_width
let visible = false;//是否显示删除按钮
const dx = 50;//滑动大于这个距离 即显示删除按钮
let vx = 0;//水平滑动的速度
let item_height = 50;
let delete_width = 100;//删除按钮的宽度
/**
 自定义滑动item
 **/
    ...
    render() {
        ...
        return (
            <View {...this._panResponder.panHandlers}
                ...
                {/*touch设置样式*/}
                <TouchableHighlight style={[styles.right, this.state.right]} onPress={() => {
                    del(id)
                }}>
